...
try {
 ...
 const reserve=Number(add.run(tender.id,'Synthetic Backup',88,'reserve-cv').lastInsertRowid);
 const reserveReview=await (await call(endpoint(reserve))).json();
 assert.equal((await call(endpoint(reserve),'POST',{...payload,revision:reserveReview.revision})).status,201);
 const reservePayload={revision:reserveReview.revision,identityKey:'reserve-person',assignmentType:'backup'};
 assert.equal((await call('/api/candidates/'+reserve+'/staffing-approval','POST',{...reservePayload,assignmentType:'invalid'})).status,400);
 assert.equal((await call('/api/candidates/'+reserve+'/staffing-approval','POST',{...reservePayload,assignmentType:'primary'})).status,409);
 assert.equal((await call('/api/candidates/'+reserve+'/staffing-approval','POST',reservePayload)).status,201);
 coverage=await (await call('/api/project-coverage')).json();
 assert.equal(coverage.rows.find(r=>r.jobId===tender.id).backups,1);
 assert.equal(coverage.rows.find(r=>r.jobId===tender.id).approved,1);
 assert.equal((await call('/api/candidates/'+first+'/staffing-approval','POST',{revision:review.revision,identityKey:'person-a',assignmentType:'backup'})).status,201);
 coverage=await (await call('/api/project-coverage')).json();
 assert.equal(coverage.rows.find(r=>r.jobId===tender.id).shortage,1);
 assert.equal(coverage.rows.find(r=>r.jobId===tender.id).backups,2);
 assert.equal(coverage.projects.find(p=>p.name==='Synthetic project').approved,0);
 assert.equal((await call('/api/candidates/'+reserve+'/staffing-approval','POST',{...reservePayload,assignmentType:'primary'})).status,201);
 assert.equal((await call('/api/candidates/'+first+'/staffing-approval','POST',{revision:review.revision,identityKey:'person-a',assignmentType:'primary'})).status,409);
 assert.equal((await call('/api/candidates/'+reserve+'/staffing-approval','POST',reservePayload)).status,201);
 assert.equal((await call('/api/candidates/'+first+'/staffing-approval','POST',{revision:review.revision,identityKey:'person-a',assignmentType:'primary'})).status,201);
