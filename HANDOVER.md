...
| **قابلية المراقبة** | فحوصات `/api/health` و`/api/ai-status`؛ سجلات الخادم لإعادة محاولات AI والترحيلات؛ تتبّع حي لاستهلاك التوكن. |
...
**المصادقة والفحص** — `POST /api/auth/login` · `GET /api/auth/me` · `GET /api/health` (عام) · `GET /api/ai-status`
